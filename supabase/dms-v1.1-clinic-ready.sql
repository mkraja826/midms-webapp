...
as $$
  ...
  select
    ...
    coalesce((select sum(amount) from today_payments where payment_category = 'op_fee'), 0),
    coalesce((select sum(amount) from today_payments where payment_category = 'xray_fee'), 0),
    coalesce((select sum(amount) from today_payments where payment_category = 'medication_fee'), 0),
    coalesce((select sum(amount) from today_payments where payment_category = 'treatment_fee'), 0),
    coalesce((select sum(amount) from today_payments where payment_category = 'pending_collection'), 0),
...
begin
  ...
  loop
    ...
    values (
      caller.clinic_id, invoice_row.id, p_patient_id, applied, p_payment_method, p_notes, p_payment_category, caller.id
    );
