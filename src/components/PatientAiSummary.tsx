...
import { askCapDentAiPatientHistory } from "@/lib/capdent-ai";
...
const PROMPTS = [
  ...
  "What treatments are recorded for this patient?",
  "What is the latest recorded diagnosis?",
  "Is a next appointment recorded?",
];
...
export function PatientAiSummary({ patientId, patientName }: Props) {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "welcome",
      role: "assistant",
      text: "I can summarize the recorded visit and treatment timeline. Phone, email, address, doctor notes, medical-history risk fields, files and X-rays are not sent to Grok in this view.",
    },
  ]);
  ...
  async function ask(question: string) {
    ...
    try {
      const result = await askCapDentAiPatientHistory(patientId, cleaned);
      setMessages((current) => [
        ...current,
        {
          id: messageId(),
          role: "assistant",
          text: result.answer || "No summary could be generated from the available recorded timeline.",
        },
      ]);
    } catch (error) {
      setMessages((current) => [
        ...current,
        {
          ...
          text: error instanceof Error
            ? error.message
            : "CapDent AI could not load this patient's recorded history right now.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <>
      <Pressable
        ...
        accessibilityLabel={`Open AI visit summary for ${displayName}`}
      ...
      >
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          <Text numberOfLines={2} style={{ color: colors.muted, marginTop: 3, fontSize: 12, lineHeight: 17 }}>
            Summarize recorded complaints, diagnoses, treatments and next-appointment history · Read-only
          </Text>
        </View>
        <View
          style={{
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
          ...
          >
            <Pressable
              accessibilityRole="button"
              accessibilityLabel="Close patient AI summary"
              ...
              >
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  <Text numberOfLines={1} style={{ color: colors.text, fontSize: 17, fontWeight: "900" }}>
                    {displayName} · AI history
                  </Text>
                  <Text style={{ color: colors.muted, fontSize: 11, marginTop: 2 }}>
                    Recorded timeline only · Read-only
                  </Text>
                </View>
                <Pressable
                  accessibilityRole="button"
              ...
              >
                ...
                {messages.map((message) => {
                  ...
                  return (
                    ...
                    >
                      <Text
                        selectable
                        style={{ color: user ? colors.white : colors.text, lineHeight: 20, fontSize: 14 }}
                      >
                        {message.text}
                      </Text>
                    </View>
                ...
                {loading ? (
                  ...
                  >
                    ...
                    <Text style={{ color: colors.muted, fontSize: 13 }}>Summarizing recorded history…</Text>
                  </View>
                ) : null}
              </ScrollView>
              ...
              >
                ...
                >
                  <TextInput
                    ...
                    multiline
                    maxLength={300}
                    placeholder="Ask about recorded visit history…"
                ...
                <Text style={{ color: colors.muted, fontSize: 10, textAlign: "center", lineHeight: 15 }}>
                  AI summarizes recorded chart data only. Verify the source record before clinical decisions.
                </Text>
              </View>
            </View>
          </View>
